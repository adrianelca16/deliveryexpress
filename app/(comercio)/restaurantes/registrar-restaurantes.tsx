...
export default function RegistrarRestaurante() {
  ...
  const onSelect = (countrySelected: { cca2: string; callingCode: string[] }) => {
    setCountry({ cca2: countrySelected.cca2, callingCode: countrySelected.callingCode });
  };
  ...
  const convertirHora24 = (hora: string) => {
    ...
    const partes = hora.replace(/(am|pm)/i, "").trim().split(":");
  ...
  const handleGuardar = async () => {
    ...
    if (faltantes.length > 0) {
      setPopupMessage(`Completa los siguientes campos:\n• ${faltantes.join('\n• ')}`);
    ...
    try {
      if (restauranteId) {
        ...
        setPopupMessage("Restaurante actualizado correctamente");
        ...
        setPopupMessage("Restaurante registrado correctamente");
        setPopupIcon("check-circle");
        setPopupVisible(true);
      ...
      setPopupMessage("No se pudo guardar la información");
      setPopupIcon("cancel");
      setPopupVisible(true);
    }
  };
  ...
  return (
    <ScreenWrapper>
      ...
      <ScrollView className="flex-1 px-5" overScrollMode="never" contentContainerStyle={{ paddingBottom: insets.bottom }}>
        {imagen && (
