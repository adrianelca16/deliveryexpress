    ...
    <ScreenWrapper gradient>
      <View className="flex-row justify-end items-center px-4 py-3 mb-2 mt-2 mx-4">
        <TouchableOpacity onPress={() => router.push("/profile")} className="items-center">
          <Ionicons name="notifications" size={32} color="#65A30D" />
        </TouchableOpacity>
      </View>

      <Animated.View entering={FadeInDown.delay(100).duration(400).springify()}>
        <Card className="flex-row gap-4 mx-4">
          <Image source={{uri: user?.foto_perfil || user?.foto_perfil_url}} className="w-24 h-24 rounded-full border-2 border-purple-200" />
          <View className="justify-between flex-1">
            <View>
              <Text className={`font-bold text-lg ${darkMode ? "text-white" : "text-black"}`}>{user?.nombre}</Text>
              <Text className={`font-bold ${disponible ? 'text-green-600' : 'text-red-500' }`}>Estado: {disponible ? 'Disponible' : 'No disponible'}</Text>
            </View>

            <View className="flex-row gap-3">
              <TouchableOpacity
                className={`py-2 px-4 rounded-full ${disponible ? 'bg-red-500' : 'bg-secondary'}`}
      ...
      <ScrollView className="px-6 mt-4" contentContainerStyle={{ paddingBottom: 100 }}>
        <Text className="text-xl font-bold mt-4 mb-3 text-center text-secondary">Órdenes asignadas</Text>
        {ordenesAsignadas.length === 0 ? (
          <Animated.View entering={FadeInDown.delay(200).duration(400).springify()}>
            <Card>
              <Text className={`${darkMode ? "text-gray-400" : "text-gray-500"} text-center`}>No tienes órdenes asignadas.</Text>
            </Card>
          </Animated.View>
        ) : (
          ordenesAsignadas.map((orden, index) => (
            <Animated.View key={orden.id} entering={FadeInDown.delay(200 + index * 100).duration(400).springify()}>
              <TouchableOpacity
                className="mb-3"
                onPress={() =>
                  router.push({
                ...
                <Card>
                  <View className="flex-row justify-between items-center">
                    <View>
                      ...
                      <Text className={`${darkMode ? "text-gray-300" : "text-gray-600"}`}>{new Date(orden.creado_en).toLocaleDateString()}</Text>
                    </View>
                    <View className="items-end">
                      <Text className="text-primary text-lg font-bold">${orden.total}</Text>
                      <Text className={`text-sm font-medium ${darkMode ? "text-gray-400" : "text-gray-500"}`}>Ver detalles</Text>
                    </View>
                  </View>
                </Card>
              </TouchableOpacity>
              ...
              {ordenActual && (
                <>
                  <View className="items-center mb-4">
                    ...
                    <Text className={`text-xl font-bold ${darkMode ? "text-white" : "text-black"}`}>Nueva Orden</Text>
                  </View>
                  <Text className={`${darkMode ? "text-gray-300" : "text-gray-700"} mb-2 text-center`}>Cliente: {ordenActual.cliente_nombre}</Text>
