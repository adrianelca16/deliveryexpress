...
export default function DireccionLista() {
  ...
  const editarDireccion = (dir: Direccion) => {
    router.push({
      pathname: "/direcciones/formulario-direccion",
      params: {
        id: dir.id.toString(),
        nombre: dir.nombre,
  ...
  return (
    <ScreenWrapper>
      <Header title="Direcciones" showBack onBack={() => router.back()} />

      <View className="px-4 mt-2">
        <Text className="text-center text-gray-500 text-base mb-4">Gestiona tus puntos de entregas frecuentes con precisión</Text>

        <CustomButton
          title="Añadir Dirección"
          onPress={() => router.push({ pathname: "/direcciones/formulario-direccion" })}
          ...
          {direcciones.length === 0 ? (
            ...
            direcciones.map((dir, index) => (
              <Animated.View key={dir.id} entering={FadeInDown.delay(100 + index * 80).duration(400).springify()} className="mb-3">
                <TouchableOpacity
                  activeOpacity={0.8}
                  onPress={() => editarDireccion(dir)}
                >
                  <Card className='border-blue-300 dark:border-blue-700' style={{ elevation: 5, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.12, shadowRadius: 8 }}>
                    <View>
